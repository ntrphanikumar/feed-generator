...
public class DeleteInactiveSKUsProcessor implements Processor {
	...
	public void process(FeedStatus feedStatus) throws Exception {
		...
		String itemDeltaURL = "http://api.remix.bestbuy.com/v1/products(itemUpdateDate>" + lastUpdateDate + "&type%20in(" + ITEM_TYPES + ")&active=false)?" + JSON_FORMAT + "&pageSize=" + PAGE_SIZE + '&' + ATTRIBUTES + '&' + API_KEY + "&page=";
		...
		if (totalProducts == 0) {
			...
			log.info(String.format("Deleting %s total products that have been updated since %s.  Processing %s total pages with a page size of %s", totalProducts, lastUpdateDate, totalPages, PAGE_SIZE));
			...
			for (int i = 0; i < totalPages; i++) {
				...
				while (productsIterator.hasNext()) {
					...
					File fileToBeRemoved = new File(dataFeedGeneratorHomeDir + DOWNLOAD_DIR + File.separator + sku + ".js");
